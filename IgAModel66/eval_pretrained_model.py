    ...
    for epoch in range(MAX_EPOCH):
        ...
        with torch.no_grad():
            ...
            if acc > max_acc:   # record best accuracy
                data.to_csv('result/'+'fold'+str(fold)+'_'+str(acc)+'.csv',index=False)
...
if __name__ == '__main__':
    ...
    resultFold=pd.DataFrame(columns=['fold'+str(i) for i in range(1,6)]+['All'])  # 存储交叉检验数据 
    fp16_precision = True
    pretrained = False
